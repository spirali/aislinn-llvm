...
///
/// \file
/// \brief This file contains the actual instruction interpreter.
...
/// Pop the last stack frame off of getECStack() and then copy the result
/// back into the result variable if we are not returning void. The
/// result variable may be the ExitValue, or the Value of the calling
/// CallInst if there was a previous stack frame. This method may
/// invalidate any getECStack() iterators you have. This method also takes
...
void Interpreter::popStackAndReturnValueToCaller(Type *RetTy,
  ...
  std::vector<ExecutionContext>&ECStack = getECStack();
  ECStack.back().free(State);
  ECStack.pop_back();
...
void Interpreter::visitAllocaInst(AllocaInst &I) {
  ...
  void *Memory = State->alloc(MemToAlloc);
  ...
  if (I.getOpcode() == Instruction::Alloca)
    getECStack().back().Allocas.push_back(Memory);
}
...
void Interpreter::visitLoadInst(LoadInst &I) {
  ...
  GenericValue Result;
  LoadValueFromMemory(Result, (GenericValue*) GVTOP(SRC), I.getType());
  SetValue(&I, Result, SF);
...
void Interpreter::visitCallSite(CallSite CS) {
  ...
  if (F && F->isDeclaration())
    ...
    case Intrinsic::vastart: { // va_start
      ...
      ArgIndex.UIntPairVal.first = getECStack().size() - 1;
...
void Interpreter::visitVAArgInst(VAArgInst &I) {
  ...
  GenericValue Dest;
  GenericValue Src = getECStack()[VAList.UIntPairVal.first]
...
GenericValue Interpreter::getOperandValue(Value *V, ExecutionContext &SF) {
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(V)) {
    ...
    return PTOGV(State->getPointerToGlobalValue(GV));
  } else {
    return SF.Values[V];
  }
}
...
void Interpreter::callFunction(Function *F,
  ...
  getECStack().push_back(ExecutionContext());
  ExecutionContext &StackFrame = getECStack().back();
...
void Interpreter::resumeRun(const GenericValue &GV)
{
      Function *F = getECStack().back().CurFunction;
      popStackAndReturnValueToCaller(F->getReturnType(), GV);
      run();
}
...
/// ConstantExpr values.
/*
 * NOTE: This methods is completely taken from ExecutionEngine.cpp
 * We just need to change access to globals
 */
GenericValue Interpreter::getConstantValue(const Constant *C) {
  ...
  case Type::PointerTyID:
    if (isa<ConstantPointerNull>(C))
      ...
      // The following line is the reason
      // why we have to duplicate this function from ExecutionEngine
      Result = PTOGV(State->getPointerToGlobalValue(
